...
class Devstack:
    ...
    def parse_config(self):
        if not self.conf.config_file:
            host = self.conf.host
            user = self.conf.user
            password = self.conf.password
        else:
            try:
                with open(self.conf.config_file) as f:
                    config = yaml.load(f)
            except IOError as e:
                raise ConfigError("Not found file {file}: {exc}".format(file=self.conf.config_file, exc=e))

            dev = config['servers']['devstack-server'][0]
            host = dev["ip"]
            user = dev["user"]
    ...
    def run(self):
        ...
        install_openstack(cfg["job"], cfg["env"], cfg["verb"], self.conf.force, None, False, self.conf.proxy)
