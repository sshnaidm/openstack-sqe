...
with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "templates", "network.yaml")) as f:
    netconf = yaml.load(f)
with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "templates", "storage.yaml")) as f:
    storconf = yaml.load(f)
with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "templates", "host_template.yaml")) as f:
    hostconf = yaml.load(f)



def erase_net(conn, lab):
    names = [i.name() for i in conn.listAllNetworks() if lab in i.name()]
...
def erase_pool(conn, lab):
    names = [i.name() for i in conn.listDefinedStoragePools() if lab in i.name()]
    for name in names:
        try:
...
def erase_vm(conn, lab):
    names = [i.name() for i in conn.listAllDomains() if lab in i.name()]
    for name in names:
        try:
...
class Network:
    ...
    def __init__(self, lab_id, config, name, **kwargs):
        ...
        self.denition = None
        self.net_ip = "192.168." + env[self.lab_id]["net_start"] + \
                      [self.config['networks'].index(i) for i in self.config['networks'] if name in i][0]
    ...
    def define(self):
        ...
        return netconf["template"].format(
            name=self.name,
            net_ip=self.net_ip,
            domain=DOMAIN_NAME,
            dhcp=dhcp_text,
...
def net_undefine1(conn, net_name):
    try:
        net = conn.networkLookupByName(net_name)
    except libvirtError:
        print >> sys.stderr, "Net %s wasn't found, nothing to delete" % net_name
